...
def test_annotate_human_cli(human_ann_vcf):
    """Test human annotation using the CLI."""
    runner = CliRunner()
    result = runner.invoke(cli.main, [HUMAN, TEST_VCF])
    assert result.exit_code == 0
    with open(TEST_VCF) as ann_test:
        assert ann_test.read() == human_ann_vcf.read()


def test_annotate_ggallus_cli(ggallus_ann_vcf):
    """Test ggallus annotation using the CLI."""
    runner = CliRunner()
    result = runner.invoke(cli.main, [GGALLUS, TEST_VCF, "--species", "ggallus"])
    assert result.exit_code == 0
    with open(TEST_VCF) as ann_test:
